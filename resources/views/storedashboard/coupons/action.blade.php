<?php $coupon = \App\Models\Coupon::whereId($id)->first();?>
<a href="{{route('coupons.edit',$id)}}" class="btn btn-sm btn-hover-bg-light m-0">
...
<div style="cursor:pointer;" onclick="deletecoupons({{$id}})" class="btn btn-sm btn-hover-bg-light mr-1">
...
<span class="switch btn btns-m switch-outline switch-icon switch-primary">
    <label>
        <input type="checkbox"  name="active" onchange="activecoupon({{$id}})" 
         @if($coupon->active == 1) checked @endif
         value="1"/>
        <span></span>
    </label>
</span>
<script>
function deletecoupons(sel){
let id = sel;
console.log(sel);
