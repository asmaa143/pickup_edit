...
class CartResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        $productId=Cart::where('callcenter_id',auth()->user()->id)->where('store_id',$storeId)->pluck('product_id');
        $products= CartProductResource::collection(Product::whereIn('id',$productId)->get());
        $cartDriver= Cart::where('callcenter_id',auth()->user()->id)->first();

        return [
            'id'       => $this->id,
            'products' => $products,
            'price'    => new CartPriceResource($cartDriver),
